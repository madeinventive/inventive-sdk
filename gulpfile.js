...
function compileAjvStandalone(schemas, validationFile) {
  console.time('* GENERATE VALIDATORS');
  const ajv = new Ajv({ schemas: schemas, code: { source: true } });
  addFormats(ajv);
  let moduleCode = standaloneCode(ajv);
  console.timeEnd('* GENERATE VALIDATORS');
  fs.writeFileSync(validationFile, moduleCode);
}

async function generateTypings(validationFile) {
...
async function buildTypes() {
  let paths = {
    ...
    typesJsonSchema: path.resolve(__dirname + '/src/__gen__/schemas'),
    validationFile: path.resolve(__dirname + '/src/__gen__/validators.js'),
  };

  /* Clear the output dir for the AJV validation code, definition and JSON Schema definitions */
  clearDir(paths.typesJsonSchema);
  fs.rmSync(paths.validationFile, { force: true });
  ...
  await generateTypings(paths.validationFile);
}
